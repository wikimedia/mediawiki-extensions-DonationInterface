		...
		<section class="auth__infobox">
			<div class="auth__infobox-inner">
				<div class="auth__intro">
					<h1 class="heading heading--h1">{{ $i18n("donorportal-login-header").text() }}</h1>
					<p class="text text--body-small">{{ $i18n("donorportal-login-text").text() }}</p>
				</div>
				<form class="auth__form send-new-link">
					<div class="cdx-text-input">
						<input id="new-checksum-link-email" ref="new-checksum-link-email" class="cdx-text-input__input" type="text"
							@input="handleInputChange" :value="donorEmail"
							:placeholder="emailPlaceholder"
							required/>
					</div>
					<button type="submit" id="request-link-button" @click="handleSubmitButtonClick"
						class="cdx-button cdx-button--action-progressive cdx-button--weight-primary"
						v-html="newLinkRequest">
					</button>
				</form>
				<p class="link-sent" id="link-sent-text" :style="`display: ${checksum_link_sent ? 'block': 'none'};`">{{ $i18n(
					"emailpreferences-new-link-sent").text() }}</p>
				<p class="error-message-text" id="error-message-text" :style="`display: ${error_message ? 'block': 'none'};`">{{ error_message }}</p>
				<a href="https://donate.wikimedia.org/" target="_blank" class="link text--body-small">{{ $i18n( "donorportal-login-problems").text() }}</a>
			</div>
		</section>
		<section class="auth__display">
			<figure>
				<img src="https://upload.wikimedia.org/wikipedia/commons/thumb/d/d1/Son_kanat_%C3%A7%C4%B1rp%C4%B1%C5%9F.jpg/2560px-Son_kanat_%C3%A7%C4%B1rp%C4%B1%C5%9F.jpg"
...
module.exports = exports = {
	...
	methods: {
		...
		handleSubmitButtonClick(e) {
			...
			if(this.$refs['new-checksum-link-email'].disabled) return;
