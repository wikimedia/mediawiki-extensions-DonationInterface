...
describe('Homme view', () => {
    ...
    it('Home view renders successfully', () => {
        ...
        expect(element.html()).toContain(HomeDataMock.address.street_address);
        expect(element.html()).toContain(HomeDataMock.address.city);
        expect(element.html()).toContain(HomeDataMock.address.country);
        expect(element.html()).toContain(HomeDataMock.name);
        expect(element.html()).toContain(HomeDataMock.email);
        expect(element.html()).toContain(HomeDataMock.donorID);
        expect(element.findAll('.donorportal-recurring-contribution').length).toBe(HomeDataMock.recurringContributions.length)
        expect(element.findAll('.donorportal-inactive-recurring').length).toBe(HomeDataMock.inactiveRecurringContributions.length)
        expect(element.find('.donorportal-recent-donation').exists).not.toBe(true)
        expect(element.findAll('.donorportal-donations-table-row').length).toBe(HomeDataMock.annualFundContributions.length + HomeDataMock.endowmentContributions.length)
    });

    it('Renders the most recent one time payment when no active or cancelled recurring is on record', () => {
        const wrapper = VueTestUtils.mount(HomeView, {
            data() {
                return {...HomeDataMock, recurringContributions: [], inactiveRecurringContributions: []}
            }
        }
        );
