...
describe( 'Cancel donations view', () => {
   ...
   it( 'Sends custom cancel reason when "Other" option is selected', async () => {
        ...
        const otherOptionRadioButton = cancelConfirmationScreen.find( '#option-other' );
        await otherOptionRadioButton.trigger( 'input' );
        await VueTestUtils.flushPromises();

        const otherOptionCustomInput = cancelConfirmationScreen.find( '#other-reason' );
        ...
        const cancelSuccessText = wrapper.find( '#recurring-contribution-cancel-success' );
        expect( cancelSuccessText.exists() ).toBe( true );
        expect( cancelSuccessText.html() )
        .toContain( `donorportal-cancel-monthly-recurring-confirmation-text:[${ HomeDataMock.result.recurringContributions[ 0 ]
        .amount_frequency_key }:[${ HomeDataMock.result.recurringContributions[ 0 ].amount_formatted },${ HomeDataMock.result
        .recurringContributions[ 0 ].currency }]]` );
    } );
