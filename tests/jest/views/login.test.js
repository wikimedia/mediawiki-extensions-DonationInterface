...
const LoginView = require( "../../../modules/ext.donationInterface.donorPortal/views/Login.vue" );
...
describe( "Login view", () => {
    ...
    it( "Api request fails on empty input", async () => {
		...
		global.mw.Api.prototype.post.mockReturnValue(
			new Promise((resolve, reject) => reject('missingparam'))
		);
    ...
    it( "Api request fails on invalid input", async () => {
		...
		// Fake an API response
		global.mw.Api.prototype.post.mockReturnValue(
			new Promise((resolve, reject) => reject('Invalid input params'))
		);
